...
def resample_laneline_in_y(input_lane, y_steps, out_vis=False,
                           interp_kind='linear', outrange_use_polyfit=False):
    """
    ...
    f_x = interp1d(input_lane[:, 1], input_lane[:, 0], kind=interp_kind, fill_value="extrapolate")
    f_z = interp1d(input_lane[:, 1], input_lane[:, 2], kind=interp_kind, fill_value="extrapolate")

    x_values = f_x(y_steps)
    z_values = f_z(y_steps)

    if outrange_use_polyfit:
        inrange_mask = np.logical_and(y_steps >= y_min, y_steps <= y_max)
        extrap_x = np.polyfit(input_lane[:, 1], input_lane[:, 0], deg=min(3, input_lane.shape[0] - 1))
        extrap_z = np.polyfit(input_lane[:, 1], input_lane[:, 2], deg=min(3, input_lane.shape[0] - 1))
        extrap_xs = np.polyval(extrap_x, y_steps)
        extrap_zs = np.polyval(extrap_z, y_steps)
        x_values[~inrange_mask] = extrap_xs[~inrange_mask]
        z_values[~inrange_mask] = extrap_zs[~inrange_mask]
