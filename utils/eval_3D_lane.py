...
class LaneEval(object):
    def __init__(self, args, logger):        
        ...
        self.y_samples = np.linspace(self.y_min, self.y_max, num=20, endpoint=False)
        self.dist_th = 1.5 #1.5
        self.ratio_th = 0.75 #0.75
        self.close_range = 40
    ...
    def bench(self, pred_lanes, pred_category, gt_lanes, gt_visibility, gt_category, raw_file, gt_cam_height, gt_cam_pitch, vis, P_g2im=None, save_dir=None):
        """
        ...
        gt_visibility_mat = np.zeros((cnt_gt, 20))
        pred_visibility_mat = np.zeros((cnt_pred, 20))
        ...
        if self.save:
            ...
            # 保存匹配的 gt_lanes
            for gt_idx in match_gt_ids:
                lane = gt_lanes[int(gt_idx)]
                visibility = gt_visibility_mat[int(gt_idx)]
                category = gt_category[int(gt_idx)]
                lane_data = {
                    'x': lane[:, 0].tolist(),  # 100 个 x 坐标
                    'z': lane[:, 1].tolist(),  # 100 个 z 坐标
                    'visibility': visibility.astype(int).tolist(),  # 100 个可见性值 (0 或 1)
                    'category': int(category)  # 类别（转换为整数）
                }
                json_data['gt_lanes'].append(lane_data)

            # 保存匹配的 pred_lanes
            for pred_idx in match_pred_ids:
                lane = pred_lanes[int(pred_idx)]
                visibility = pred_visibility_mat[int(pred_idx)]
                category = pred_category[int(pred_idx)]
                lane_data = {
                    'x': lane[:, 0].tolist(),  # 100 个 x 坐标
                    'z': lane[:, 1].tolist(),  # 100 个 z 坐标
                    'visibility': visibility.astype(int).tolist(),  # 100 个可见性值 (0 或 1)
                ...
                json_data['pred_lanes'].append(lane_data)

            # 构建保存路径
            # 提取 raw_file 的目录部分（去除 'training/'）和文件名
            relative_path = raw_file.split(os.sep, 1)[1] if raw_file.startswith(('training' + os.sep, 'validation' + os.sep)) else raw_file
            subdir = os.path.dirname(relative_path)  # 如 segment-9015546800913584551_4431_180_4451_180_with_camera_labels
            filename = os.path.splitext(os.path.basename(raw_file))[0] + '.json'  # 如 150914215095030100.json
            ...
            self.logger.info(f'Saved matched lanes JSON to {json_path}')
    ...
    def bench_one_submit_ddp(self, pred_lines_sub, gt_lines_sub, model_name, prob_th=0.5, vis=True):
        ...
        if save_dir:
            ...
            self.logger.warning("vis_dir not specified, JSON files will not be saved.")
            save_dir = None
        ...
        for i, pred in enumerate(json_pred):
            ...
            pred_lanes = pred['pred_laneLines']  #(40,20,3)
            pred_lanes_prob = pred['pred_laneLines_prob'] #(40,21)
            ...
            # Note: non-lane class is already filtered out in compute_3d_lanes_all_category()
                ...
                pred_lanes = [pred_lanes[ii] for ii in range(len(pred_lanes_prob)) if max(pred_lanes_prob[ii]) > prob_th] #(4,20,3)
                pred_lanes_prob = [prob for k, prob in enumerate(pred_lanes_prob) if max(prob) > prob_th] #(4,21)
