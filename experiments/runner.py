        ...
        for epoch in range(args.start_epoch, args.nepochs):
            ...
            for i, extra_dict in enumerate(train_loader):
                ...
                if gpu_available():
                    ...
                    point_cloud = extra_dict['point_cloud']
                image = image.contiguous().float()
                point_cloud = point_cloud.contiguous().float()
                # Run model
                optimizer.zero_grad()
                output = model(image=image, point_cloud=point_cloud, extra_dict=extra_dict, is_training=True)
    ...
    def validate(self, model, **kwargs):
        ...
        with torch.no_grad():
            ...
            for i, extra_dict in enumerate(loader):
                ...
                output = model(image=image, point_cloud=point_cloud, extra_dict=extra_dict, is_training=False)
                all_line_preds = output['all_line_preds'] # in ground coordinate system #(2,2,40,60)
                all_cls_scores = output['all_cls_scores'] #(2,2,40,21)
                ...
                for j in range(num_el):
                    ...
                    # pred in ground
                    lane_pred = all_line_preds[j].cpu().numpy() #[40,60]
