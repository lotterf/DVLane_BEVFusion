...
class SparseInsDecoder(nn.Module):  #特征图 → 实例分支 + 掩码分支 → 预测车道实例 mask、类别、分数。
    def __init__(self, cfg) -> None:
        ...
        if self.pe_enc_mtd is None: #位置编码方式，可选：SE | cat | sum2feat | None
            pass
    ...
    @torch.no_grad()
    def compute_coordinates(self, x): #生成归一化 2D 网格坐标,BEV 中的 x,y 坐标位置编码
    ...
    def forward(self, features, is_training=True,
        ...
        # feat : B x C x h x w
        features = torch.cat([coord_features, features], dim=1) #原始特征图加上坐标位置特征。

        if self.pe_enc_mtd is None: #加入 3D 位置编码
            pass
        elif self.pe_enc_mtd == 'SE':
        ...
        inst_output = self.inst_branch(features, is_training=is_training) #实例分支,预测每个实例的类别、分数、kernel。
        output.update(inst_output)

        if is_training:
            mask_features = self.mask_branch(features) #掩码分支（只在训练时）
        ...
        if is_training: # kd need this  Hungarian 匹配,计算 mask、类别、分数的损失。
