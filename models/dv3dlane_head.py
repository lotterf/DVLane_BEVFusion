...
from .pred_utils import build_cls_reg_heads, build_refpts_pred_layer, build_fc_layer
...
class DV3DLaneHead(nn.Module):
    def __init__(self, args,
        ...
        super().__init__()
        self.args = args
        self.num_lidar_feat = num_lidar_feat #LiDAR 点特征维度
        self.insert_lidar_feat_before_img = insert_lidar_feat_before_img #是否在图像特征前插入点云投影特征
    ...
    def forward(self, input_dict, is_training=True):
        output_dict = {}
        bev_feats = input_dict['x'] #torch.Size([1, 256, 250, 300])
        ...
        output_dict.update(sparse_bev_output)

        # generate 2d pos emb
        B, C, H, W = bev_feats[0].shape
        masks = bev_feats[0].new_zeros((B, H, W))

        # TODO use actual mask if using padding or other aug  生成 2D 正弦位置编码
        sin_embed = self.pos_encoding_2d(masks)
        sin_embed = self.adapt_pos3d(sin_embed)

        query_bev_pos = sparse_bev_output['inst_features'] # BxNxC  从稀疏实例分支拿到查询初始特征
        query_pos = query_bev_pos.unsqueeze(2) + self.point_embedding.weight[None, None, ...] #给 query 加上 point embedding 偏置
        query_pos_embeds = self.query_embedding(query_pos).flatten(1, 2)

        reference_points = self.reference_points(query_pos_embeds) #生成参考点
        reference_points = reference_points.sigmoid()
        query_cont_embeds = torch.zeros_like(query_pos_embeds)

        # pos
        assert sparse_bev_output['iam'].shape[-1] == W
        assert sparse_bev_output['iam'].shape[-2] == H

        #把多尺度卷积特征 mlvl_feats 处理成 transformer 可接收的输入
        ...
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=query_bev_pos.device)
        ...
        if is_training:
            ...
            # if self.depth_net is not None:  #从图像特征 x 预测深度分布（depth map），并且可选返回上下文特征（context feature）
